...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($start_date) || empty($end_date) || !is_numeric($bcv_rate) || $bcv_rate <= 0 || !is_numeric($days_in_period) || $days_in_period <= 0) {
        ...
        try {
            ...
            $stmt_employees = $pdo->query("SELECT id, cedula, full_name, salario_base_mensual_usd FROM employees WHERE is_active = 1");
            $employees = $stmt_employees->fetchAll();

            if (empty($employees)) {
                $message = 'No hay empleados activos para calcular la nómina.';
                $message_type = 'info';
                // Eliminar el período recién creado si no hay empleados
                $pdo->prepare("DELETE FROM payroll_periods WHERE id = :id")->execute([':id' => $payroll_period_id]);
            } else {
                // 3. Calcular y guardar los detalles de la nómina para cada empleado
                ...
                // 4. Actualizar el estado del período a 'calculated'
                ...
                <form method="POST" action="">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.0001" class="form-control" id="bcv_rate" name="bcv_rate" placeholder="Ej: 101.0822" required>
                        </div>
                        <div class="col-md-6">
